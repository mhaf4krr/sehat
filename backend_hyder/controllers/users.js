    ...
    try {
        ...
        if(user.length == 0){
            ...
            if(result.length != 0){
                ...
                await mailer.sendEmail(user.EMAIL,"OTP for LOGIN",`Howdy, ${user.FULL_NAME}. Your OTP for ${user.PHONE} is ${otp}`)
            ...
            let otp_generated =  OTP.generate(4,{digits:true,alphabets:false,upperCase:false,specialChars:false})
            ...
            await mailer.sendEmail(user.EMAIL,"OTP for LOGIN",`Howdy, ${user.FULL_NAME}. Your OTP for ${user.PHONE} is ${otp_generated}`)
